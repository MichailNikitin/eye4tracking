...
import cv2
import torch# YOLOv5 🚀 by Ultralytics, GPL-3.0 license
...
@torch.no_grad()
def inicial(weights='barhuman_yolov5.pt',  # model.pt path(s)
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_txt:  # Write to file
                        ...
                        with open(txt_path + '.txt', 'a') as f:
                            f.write(('%g ' * len(
                                line)).rstrip() % line + " " + f'Speed: %.1fms pre-process, %.1fms inference, %.1fms NMS per image at shape {(1, 3, *imgsz)}' % t + '\n')
...
def parse_opt():
    ...
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[128],
                        help='inference size h,w')  # 640
